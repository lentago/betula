...
#     Grafana Cloud Loki credentials (manually scp'd from a workstation — this
...
readonly REPO_URL="https://github.com/lentago/betula.git"
...
if [[ ! -f "$ENV_FILE" ]]; then
  ...
  echo "       and fill in GRAFANA_CLOUD_LOGS_{HOST,USER,TOKEN} before bootstrapping."
  exit 1
fi
...
cp "${CLONE_PATH}/scripts/rotate_logs.sh"              "${LIVE_DIR}/rotate_logs.sh"
