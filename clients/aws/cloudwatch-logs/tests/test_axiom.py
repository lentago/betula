...
from cloudwatch_shipper.axiom import AxiomIngestClient, AxiomIngestError  # noqa: E402
...
class AxiomFramingTest(unittest.TestCase):
    ...
    def test_ingest_url(self):
        self.assertEqual(
            ...
            "https://api.axiom.co/v1/datasets/cjp-solidago-ask/ingest",
        )
    ...
    def test_headers_carry_bare_token_as_single_bearer(self):
        self.client.ship([{"a": 1}])
        headers = self.transport.calls[0]["headers"]
        # The client builds "Bearer <token>" itself from a BARE token — never a
        # doubled "Bearer Bearer ..." from a pre-built header string.
    ...
    def test_body_is_gzipped_ndjson(self):
        ...
        self.assertEqual(raw[:2], b"\x1f\x8b")  # gzip magic
...
class AxiomBatchingTest(unittest.TestCase):
    def test_batches_at_batch_size(self):
        ...
        self.assertEqual(len(transport.calls), 3)  # 2, 2, 1
        sizes = [
...
class AxiomConfigTest(unittest.TestCase):
    ...
    def test_from_env_reads_dataset_and_token(self):
        env = {"AXIOM_DATASET": "cjp-solidago-ask", "AXIOM_API_TOKEN": "xaat-x"}
        client = AxiomIngestClient.from_env(env=env, transport=FakeTransport())
        self.assertEqual(client.dataset, "cjp-solidago-ask")
