"""Axiom ingest client — gzip + ndjson HTTP POST, dependency-light.

Identical in shape to the ALB client's Axiom emitter
(``clients/aws/alb-logs/alb_shipper/axiom.py``) and to the destination shape of
the Firewalla client's Fluent Bit HTTP output (``fluent-bit/fluent-bit.conf``):
TLS POST to the Axiom ingest endpoint, an ``Authorization: Bearer <token>``
header this client builds itself, and gzip compression. Where Fluent Bit sends
a JSON array, this client sends newline-delimited JSON
(``application/x-ndjson``), which Axiom accepts and which streams
batch-by-batch without buffering an array in memory.

Uses only the Python standard library (``urllib``) so it adds no dependencies
to the Lambda package. The token is read as a **bare** value from an env var
(or injected config) and is never hard-coded; the ``Bearer`` prefix is added
here. Do **not** feed this a ``Authorization Bearer <token>`` header-string —
that FireLens/ECS convention would produce a broken double-``Bearer`` header
(the exact trap documented in solidago's ``modules/secrets/main.tf``). The HTTP
transport is injectable so unit tests exercise batching and framing without a
live Axiom endpoint.
"""
...
# Firewalla client's fluent-bit.conf for consistency across the emitters.
...
class AxiomIngestClient:
    ...
    def from_env(cls, env=None, **kwargs):
        ...
        Reads the dataset from ``AXIOM_DATASET`` and the **bare** token from the
        first set variable in :data:`TOKEN_ENV_VARS`. Keeping the token in the
    ...
    def _post_batch(self, events):
        ...
        headers = {
            # Bare token from the env; the Bearer prefix is added HERE — never
            # pass a pre-built "Authorization Bearer ..." string (see docstring).
    ...
    def ship(self, events):
        ...
        the failure and lets the CloudWatch delivery be retried rather than
